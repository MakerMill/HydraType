...
final readonly class Trim implements MutatorInterface
{
    public function __construct(private string $characters = " \t\n\r\0\x0B")
    {
        if ($characters === '') {
            throw new InvalidArgumentException('Trim character list must not be empty.');
        }
    }

    public function compile(string $inputExpression): string
    {
        return 'trim((string) ' . $inputExpression . ', ' . PhpLiteral::string($this->characters) . ')';
    }
