...
final readonly class RightTrim implements MutatorInterface
{
    public function __construct(private string $characters = " \t\n\r\0\x0B")
    {
        if ($characters === '') {
            throw new InvalidArgumentException('RightTrim character list must not be empty.');
        }
    }

    public function compile(string $inputExpression): string
    {
        return 'rtrim((string) ' . $inputExpression . ', ' . PhpLiteral::string($this->characters) . ')';
    }
