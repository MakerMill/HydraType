...
final readonly class NotOneOf implements AssertionInterface
{
    ...
    {
        return 'Value must not be one of the configured values';
    }
    ...
    {
        if ($values === []) {
            throw new InvalidArgumentException('NotOneOf values must not be empty.');
        }

        $validated = [];
        foreach ($values as $value) {
            if (!is_string($value) && !is_int($value) && !is_float($value) && !is_bool($value)) {
                throw new InvalidArgumentException('NotOneOf values must be strings, integers, floats, or booleans.');
            }
