...
$mapper = $autoMapper->getMapper('array', CompetitorRecord::class);

$map = static function (int $count) use ($mapper, $data): CompetitorRecord {
    for ($i = 0; $i < $count; $i++) {
...
$timings = BenchmarkRunner::measure(
    [
        'JoliCode AutoMapper 10' => new BenchmarkCase(
            static fn (): CompetitorRecord => $map($operations),
            $operations,
            static function (mixed $object) use ($expectedChecksum): void {
            ...
            static fn (): CompetitorRecord => $map($warmupOperations),
